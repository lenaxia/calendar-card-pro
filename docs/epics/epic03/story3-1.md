...
- **MDI icon for the panel**: `mdiViewWeek`, `mdiCalendarWeek`, or `mdiTableLarge` — pick one that fits visually with the existing card icons (existing panels use `mdiCalendarMonth`, `mdiPalette`, `mdiCog`). **Add the import** to the existing `from '@mdi/js'` block at `editor.ts:22-31`. For example: `import { mdiCalendarMonth, mdiPalette, ..., mdiViewWeek } from '@mdi/js';`.
