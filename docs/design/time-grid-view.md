...
- **FR-11.1** List-view render path is byte-for-byte unchanged. Bundle-size delta target: ≤ +35 KB minified (current: 282 KB → cap 317 KB). Estimate breakdown: utils/grid.ts ~5 KB, render-grid.ts ~7 KB, calendar-card-pro.ts additions ~3 KB, ReactiveControllers (NowLine + ResponsiveColumns + IntersectionObserver) ~5 KB, editor additions (with select fields + min/max constraints) ~4 KB, validation block + i18n ~2 KB, CSS additions ~3 KB, robustness layer (try/catch error boundary + Intl formatter cache + accent_color var() preservation + DRY documentation) ~3 KB. Cap raised from +20 KB → +25 KB → +30 KB → +35 KB across worklog-0020 iterations (FR-2.6 implementation, exhaustiveness, validation, IntersectionObserver, a11y, controllers, then E-phase robustness/perf). The cap is a process accountability number — not an external constraint; raising transparently is preferable to compressing below the correctness/robustness threshold.
...
| R-5  | Bundle size inflation                                                                                       | Low                | Low    | Track `dist/calendar-card-pro.js` byte size; cap +35 KB (raised from +20 → +25 → +30 → +35 across worklog-0020 iterations as FR-2.6, validation, controllers, a11y, and E-phase robustness fixes were added) |
...
- `npm run build` succeeds. Bundle size delta ≤ +35 KB minified.
- `npm test` passes (new).
