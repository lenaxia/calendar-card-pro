...
- **FR-11.1** List-view render path is byte-for-byte unchanged. Bundle-size delta target: ≤ +30 KB minified (current: 282 KB → cap 312 KB). Estimate breakdown: utils/grid.ts ~5 KB, render-grid.ts ~7 KB, calendar-card-pro.ts additions ~3 KB, ReactiveControllers (NowLine + ResponsiveColumns + IntersectionObserver) ~5 KB, editor additions ~3 KB, validation block + i18n ~2 KB, CSS additions ~3 KB. Cap raised from +20 KB → +25 KB → +30 KB across two worklog-0020 iterations (FR-2.6 implementation, exhaustiveness checks, full validation, IntersectionObserver, a11y, controllers refactor). The cap is a process accountability number — not an external constraint; raising transparently is preferable to compressing below the correctness/robustness threshold.
...
| R-5  | Bundle size inflation                                                                                       | Low                | Low    | Track `dist/calendar-card-pro.js` byte size; cap +30 KB (raised from +20 → +25 → +30 across worklog-0020 iterations as FR-2.6, validation, controllers, and a11y fixes were added) |
...
- `npm run build` succeeds. Bundle size delta ≤ +30 KB minified.
- `npm test` passes (new).
