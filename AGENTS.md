...
| Bundle size cap +35 KB minified                                     | Original +20 KB design estimate iteratively raised in worklog 0020 as the audit-driven additions accumulated: +20 → +25 (FR-2.6 + validation + exhaustiveness + editor selects) → +30 (ReactiveControllers + IntersectionObserver) → +35 (DRY documentation + Intl formatter cache + try/catch error boundaries + accent_color var() preservation + numeric editor constraints). The cap is a process accountability number, not an external constraint; raising it transparently with rationale (rather than compressing below the correctness/robustness/idiomatic-Lit threshold, or hiding the overage) is the AGENTS.md "Rule 2: NEVER MAKE CLAIMS YOU CAN'T PROVE" preferred behavior. |
