...
| Bundle size cap +30 KB minified                                     | Original +20 KB design estimate was raised twice in worklog 0020: first to +25 KB after the audit added FR-2.6 hidden-pill / clipped-indicators, full-validation, exhaustiveness, and editor select fields; then to +30 KB after the ReactiveControllers refactor (NowLineController + ResponsiveColumnsController + IntersectionObserver + happy-dom test environment add ~3 KB on top). The cap is a design-process accountability number, not an external constraint; raising it transparently is preferable to compressing the implementation below the threshold of correctness/robustness/idiomatic-Lit. |
