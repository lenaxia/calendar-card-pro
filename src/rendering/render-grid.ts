...
const buildSwipeEnd =
  ...
  (e: TouchEvent): void => {
    ...
    const startX = Number(target.dataset.swipeX ?? 0);
...
function renderEventBlock(
  ...
  const bgColor = EventUtils.getEntityAccentColorWithOpacity(event._entityId, config, 20, event);
...
function renderDayHeaderWeather(
  ...
  return html`
    <span class="ccp-grid-day-header-weather" style="font-size: ${fontSize};">
      ...
      ${showLowTemp
        ? html`<span class="ccp-grid-weather-low">/${forecast.templow}°</span>`
        : nothing}
    </span>
  `;
}
